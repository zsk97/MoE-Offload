...
def repeat_kv(hidden_states: torch.Tensor, n_rep: int) -> torch.Tensor:
    ...
    num_local_key_value_heads, seqlen, head_dim) to (batch, num_attention_heads, seqlen, head_dim)
    """
    batch, num_local_key_value_heads, slen, head_dim = hidden_states.shape
    if n_rep == 1:
        return hidden_states
    hidden_states = hidden_states[:, :, None, :, :].expand(batch, num_local_key_value_heads, n_rep, slen, head_dim)
    return hidden_states.reshape(batch, num_local_key_value_heads * n_rep, slen, head_dim)
...
class MixtralAttention(nn.Module):
    ...
    def __init__(self, config: MixtralConfig, layer_idx: Optional[int] = None):
        ...
        self.q_proj = ColumnParallelLinear(
            ...
            gather_output=False, init_method=default_linear_init)
        ...
        self.o_proj = RowParallelLinear(
            ...
            input_is_parallel=True, init_method=default_linear_init)
    ...
    def _shape(self, tensor: torch.Tensor, seq_len: int, bsz: int):
        return tensor.view(bsz, seq_len, self.num_local_heads, self.head_dim).transpose(1, 2).contiguous()
    ...
    def forward(
        ...
        **kwargs,
        ...
        query_states = query_states.view(bsz, q_len, self.num_local_heads, self.head_dim).transpose(1, 2)
        key_states = key_states.view(bsz, q_len, self.num_local_key_value_heads, self.head_dim).transpose(1, 2)
        value_states = value_states.view(bsz, q_len, self.num_local_key_value_heads, self.head_dim).transpose(1, 2)
        ...
        if attn_weights.size() != (bsz, self.num_local_heads, q_len, kv_seq_len):
            raise ValueError(
                f"Attention weights should be of size {(bsz, self.num_local_heads, q_len, kv_seq_len)}, but is"
                f" {attn_weights.size()}"
            )
        ...
        if attn_output.size() != (bsz, self.num_local_heads, q_len, self.head_dim):
            raise ValueError(
                f"`attn_output` should be of size {(bsz, self.num_local_heads, q_len, self.head_dim)}, but is"
                f" {attn_output.size()}"
            )
...
class MixtralModel(MixtralPreTrainedModel):
    ...
    def __init__(self, config: MixtralConfig):
        ...
        self.embed_tokens = ParallelEmbedding(config.vocab_size, config.hidden_size, self.padding_idx, init_method=default_linear_init)
...
class MixtralForCausalLM(MixtralPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.vocab_size = config.vocab_size
        self.lm_head = ColumnParallelLinear(config.hidden_size, config.vocab_size, bias=False, init_method=default_linear_init)
    ...
    def load_state_dict_from_safetensor_files(self, safetensor_files: List[str]):
        local_ckpt = {}
        # expert part
        is_expert_part = lambda key: "block_sparse_moe.experts" in key
        n_local_experts = self.num_experts // self.mp_size
        local_experts = [i for i in range(n_local_experts*self.mp_rank, n_local_experts*(self.mp_rank+1))]
        
        # columnparallel part: qkv, lm_head
        def is_ColumnParallel_module(key):
            keywords = ['.q_proj.weight', '.k_proj.weight', '.v_proj.weight', 'lm_head.weight']
            for keyword in keywords:
                if keyword in key:
                    return True
            return False
        def is_RowParallel_module(key):
            return'.o_proj.weight' in key or 'embed_tokens' in key
        
        for file in safetensor_files:
            crt_ckpt = load_file(file)
            for key, val in crt_ckpt.items():
                if "model.layers." in  key:
                    layer_id = int(key.split("model.layers.")[1].split('.')[0])
                    if layer_id >= self.__config.num_hidden_layers:
                        continue
                if is_expert_part(key):
                    expert_idx = int(key.split('block_sparse_moe.experts.')[1].split('.')[0])
                    if expert_idx in local_experts:
                        local_ckpt[key] = val
                elif is_ColumnParallel_module(key):
                ...
                elif is_RowParallel_module(key):
        ...
        def is_qkv_lm_head_module(name):
            ...
            for keyword in keywords:
                if keyword in name:
                    return True
            return False
        is_o_module = lambda name: '.o_proj.weight' in name
